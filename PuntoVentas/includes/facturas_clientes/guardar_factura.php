...
if ($accion == "alta") {
    $query_operacion = "INSERT INTO facturas (codfactura, numfactura, fecha, iva, nombreCliente, estado, borrado, remito) VALUES ('', '$numfactura', '$fecha', '$iva', '$nombreCliente', '1', '0', '$remito')";
...
if ($accion == "modificar") {
    ...
    $act_albaran = "UPDATE facturas SET codcliente='$nombreCliente', fecha='$fecha', iva='$iva', remito='$remito', numfactura='$numfactura' WHERE codfactura='$codfactura'";
    ...
    $contador = 0;
    while ($contador < @$conn->num_rows($rs_lineas)) {
    ...
    $contador = 0;
    $baseimponible = 0;
    while ($contador < @$conn->num_rows($rs_lineastmp)) {
...
if ($accion == "baja") {
    ...
    $nombreCliente = @mysql_result($rs_mostrar, 0, "nombreCliente");
